...
async function getPDFCasaDiaz()
{
    var xmlString = await xml.serializeXML(xml64)
            ...
            [
                {
                    table: {
                        body: [
                            ...
                            [
                                {border: [true, false, true, false], text: `TIPO DE COMPROBANTE: ${attributes.TipoDeComprobante}`, style: 'textotabla' , alignment: 'left'},
                            ],
                            [
                                {border: [true, false, true, true], text: `RÉGIMEN FISCAL: ${emisor.attributes.RegimenFiscal} `, style: 'textotabla' , alignment: 'left'},
                            ],
                        ]
                    },
                    layout: {
        ...
        table: {
            ...
            body: [
                ...
                [
                    ...
                    {text: `${receptor.attributes.UsoCFDI}`, style: 'textoTablaCliente', alignment: 'center'}, 
                ],
            ]
        },
        layout: {
    ...
    {
        conceptosArray[cCount] = [
            {text: `${conceptos.elements[i].attributes.ClaveProdServ}`, alignment: 'center', style: 'textoTablaCliente'},
            {text: `${conceptos.elements[i].attributes.Cantidad}`, alignment: 'center', style: 'textoTablaCliente'},
            {text: `${conceptos.elements[i].attributes.ClaveUnidad}`, alignment: 'center', style: 'textoTablaCliente'},
    ...
    var totales = 
    {
        columns: [
            {
                width: "*",
                text: `Moneda: ${attributes.Moneda} - Los códigos asignados para las transacciones \nen que intervenga ninguna moneda`,
                style: 'moneda'
            },
            {
                width: 200,
    ...
    fs.writeFileSync(imageQR, buffer);
    ...
    var codigos = {
        table: {
            ...
            body: [
                [
                    {image: imageQR, width: 120, height: 120, alignment: 'center', verticalAlign: 'middle'},
    ...
    for(var i=0; i<mercanciasAux.length; i++)
    {
        ...
        mercanciasArray[mCount] = [
            {text: `${mercanciasAux[i].attributes.BienesTransp}`, alignment: 'center', style: 'ubicacionesTexto'},
